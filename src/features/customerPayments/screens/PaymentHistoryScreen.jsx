...
export default function PaymentHistoryScreen({ navigation }) {
    ...
    const handleStatusChange = useCallback((payment, status) => {
        ...
        Alert.alert("Cập nhật trạng thái", `Bạn muốn chuyển giao dịch này sang "${statusLabel[status]}"?`, [
    ...
    if (loading && paymentHistory.length === 0) {
        return (<View className="flex-1 items-center justify-center bg-slate-50">
        <ActivityIndicator size="large" color={Colors.indigo600}/>
      </View>);
    }
    if (error && paymentHistory.length === 0) {
        return (<View className="flex-1 items-center justify-center bg-slate-50 p-6">
        <Text className="text-center text-red-500">{error}</Text>
      </View>);
    }
    return (<View className="flex-1 bg-slate-50 px-4 pt-4">
      ...
      <FlatList data={paymentHistory} keyExtractor={(item) => item._id} renderItem={({ item }) => (<PaymentHistoryItem payment={item} onPress={handlePressPayment} showStatusActions={isAdmin} isUpdating={updatingPaymentId === item._id} onStatusChange={handleStatusChange}/>)} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 24 }} ListEmptyComponent={!loading ? (<View className="mt-12 items-center justify-center">
              <Text className="text-slate-500">
                {isAdmin
                ? "Chưa có giao dịch thanh toán nào trong hệ thống"
                : "Bạn chưa có lịch sử thanh toán nào"}
              </Text>
            </View>) : null} refreshing={loading} onRefresh={() => dispatch(fetchPaymentHistoryThunk(isAdmin ? "all" : "my"))}/>
    </View>);
}
