...
import { ArrowLeft, Broom, Tag, CurrencyDollar, Ruler, PencilSimple, Trash, CheckCircle, XCircle, } from 'phosphor-react-native';
import { useAppDispatch, useAppSelector } from '@/app/store';
import { fetchServiceByIdThunk, deleteServiceThunk, clearSelectedService, clearServiceError, } from '../servicesSlice';
import { Colors, shadowCard, shadowOutline, shadowFloating, pressedStyleSmall, layoutContainer, labelStyle, } from '@/theme/tokens';
...
export default function ServiceDetailScreen({ navigation, route }) {
    ...
    return (<SafeAreaView className="flex-1 bg-page">
      ...
      {service && (<ScrollView contentContainerStyle={layoutContainer} contentContainerClassName="px-6 pb-8">
          {/* Service icon + name */}
          <View className="mb-6 items-center">
            {service.image ? (<Image source={{ uri: service.image }} className="mb-4 h-44 w-full rounded-3xl bg-slate-100" resizeMode="cover"/>) : (<View className="mb-4 h-20 w-20 items-center justify-center rounded-full bg-indigo-50">
            ...
            <View className="mt-2 flex-row items-center gap-1.5">
              {service.active ? (<CheckCircle size={16} color="#22C55E" weight="fill"/>) : (<XCircle size={16} color="#DC2626" weight="fill"/>)}
              <Text className="text-sm font-bold" style={{ color: service.active ? '#22C55E' : '#DC2626' }}>
          ...
          <SectionCard title="THÔNG TIN">
            ...
            <InfoRow icon={CurrencyDollar} label="Giá" value={formatPrice(service.price)} valueColor={Colors.indigo600}/>
            <InfoRow icon={Ruler} label="Đơn vị" value={service.unit}/>
          </SectionCard>

          {/* Admin actions */}
          {isAdmin && (<View className="mt-2 gap-3">
              {/* Edit */}
              <Pressable onPress={() => navigation.navigate('ServiceForm', {
                    serviceId: service._id,
                })} className="flex-row items-center justify-center gap-2 rounded-xl border-2 border-slate-200 bg-white px-6 py-3.5" style={({ pressed }) => [shadowOutline, pressedStyleSmall(pressed)]}>
              ...
              {/* Delete */}
              <Pressable onPress={handleDelete} disabled={isLoading} className="flex-row items-center justify-center gap-2 rounded-xl border-2 border-red-200 bg-white px-6 py-3.5" style={({ pressed }) => [
