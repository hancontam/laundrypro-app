...
import { Colors, shadowCTA, shadowFloating, pressedStyle, pressedStyleSmall, layoutContainer, labelStyle, } from '@/theme/tokens';
    ...
    return (<SafeAreaView className="flex-1 bg-page">
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1">
        <ScrollView contentContainerStyle={layoutContainer} contentContainerClassName="px-6 pb-8 pt-2" keyboardShouldPersistTaps="handled">
          {displayError && (<View className="mb-4 rounded-xl bg-red-50 px-4 py-3">
          ...
          <View className="mb-6">
            ...
            <View className="flex-row items-center rounded-xl border border-slate-200 bg-slate-50 px-3">
              ...
              <TextInput className="ml-3 flex-1 py-3.5 text-base font-medium text-slate-900" placeholder="Nhập mật khẩu hiện tại" placeholderTextColor={Colors.slate400} secureTextEntry value={currentPassword} onChangeText={(text) => {
          ...
          <View className="mb-6">
            ...
            <View className="flex-row items-center rounded-xl border border-slate-200 bg-slate-50 px-3">
              <LockKey size={18} color={Colors.slate400} weight="bold"/>
              <TextInput className="ml-3 flex-1 py-3.5 text-base font-medium text-slate-900" placeholder="Tối thiểu 8 ký tự" placeholderTextColor={Colors.slate400} secureTextEntry value={newPassword} onChangeText={(text) => {
            setNewPassword(text);
            setLocalError(null);
            dispatch(clearProfileError());
        }}/>
          ...
          <View className="mb-6">
            ...
            <View className="flex-row items-center rounded-xl border border-slate-200 bg-slate-50 px-3">
              <LockKey size={18} color={Colors.slate400} weight="bold"/>
              <TextInput className="ml-3 flex-1 py-3.5 text-base font-medium text-slate-900" placeholder="Nhập lại mật khẩu mới" placeholderTextColor={Colors.slate400} secureTextEntry value={confirmPassword} onChangeText={(text) => {
