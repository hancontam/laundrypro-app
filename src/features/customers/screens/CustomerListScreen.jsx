...
import { fetchCustomersThunk, loadMoreCustomersThunk, selectAllCustomers, } from '../customersSlice';
...
function CustomerCard({ customer, onPress, }) {
    return (<Pressable onPress={onPress} className="mb-3 flex-row items-center justify-between rounded-2xl border border-slate-100 bg-white p-4" style={({ pressed }) => [shadowCard, pressedStyle(pressed)]}>
      <View className="flex-row items-center gap-3">
        ...
        <View>
          ...
          <View className="mt-1.5 flex-row items-center gap-2">
            {customer.status === 'active' ? (<View className="rounded bg-green-50 px-2 py-0.5">
                <Text className="text-[10px] font-bold text-green-700">HOẠT ĐỘNG</Text>
              </View>) : (<View className="rounded bg-red-50 px-2 py-0.5">
                <Text className="text-[10px] font-bold text-red-700">ĐÃ KHÓA</Text>
              </View>)}
            {customer.isVerified && (<ShieldStar size={14} color="#22C55E" weight="fill"/>)}
          </View>
        </View>
      </View>
...
export default function CustomerListScreen({ navigation }) {
    ...
    const { isLoading, isLoadingMore, pagination, error } = useAppSelector((state) => state.customers);
    useEffect(() => {
    ...
    return (<SafeAreaView className="flex-1 bg-page">
      ...
      <FlatList data={customers} keyExtractor={(item) => item._id} renderItem={({ item }) => (<CustomerCard customer={item} onPress={() => navigation.navigate('CustomerDetail', { customerId: item._id })}/>)} contentContainerStyle={[layoutContainer, { paddingHorizontal: 24 }]} contentContainerClassName="pb-24 pt-2" ListEmptyComponent={!isLoading ? <EmptyState /> : null} refreshControl={<RefreshControl refreshing={isLoading} onRefresh={handleRefresh} colors={[Colors.indigo600]} tintColor={Colors.indigo600}/>} onEndReached={handleLoadMore} onEndReachedThreshold={0.3} ListFooterComponent={renderFooter}/>

      <Pressable onPress={() => navigation.navigate('CustomerForm')} className="absolute bottom-28 right-6 h-14 w-14 items-center justify-center rounded-full bg-slate-900" style={({ pressed }) => [shadowCTA, pressedStyleSmall(pressed)]}>
