...
import { createCustomerThunk, updateCustomerThunk, clearCustomersError, selectCustomerById, } from '../customersSlice';
...
export default function CustomerFormScreen({ navigation, route }) {
    ...
    const existingCustomer = useAppSelector((state) => customerId ? selectCustomerById(state, customerId) : null);
    ...
    const isValid = (isEditing || phone.trim().length >= 10) && name.trim().length >= 1;
    const handleSave = useCallback(async () => {
        ...
        if (isEditing && customerId) {
            ...
            const result = await dispatch(createCustomerThunk({
    ...
    return (<SafeAreaView className="flex-1 bg-page">
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1">
        <ScrollView contentContainerStyle={layoutContainer} contentContainerClassName="px-6 pb-8 pt-2" keyboardShouldPersistTaps="handled">
          ...
          <View>
            ...
            <View className="mb-6">
              ...
              <View className={`flex-row items-center rounded-xl border border-slate-200 px-3 ${isEditing ? 'bg-slate-100' : 'bg-slate-50'}`}>
                <Phone size={18} color={Colors.slate400} weight="bold"/>
                <TextInput className={`ml-3 flex-1 py-3.5 text-base font-medium ${isEditing ? 'text-slate-500' : 'text-slate-900'}`} placeholder="0901234567" placeholderTextColor={Colors.slate400} keyboardType="phone-pad" value={phone} onChangeText={setPhone} editable={!isEditing}/>
              </View>
              {isEditing && (<Text className="mt-1 text-xs text-slate-400">Không thể sửa đổi số điện thoại</Text>)}
            </View>
            ...
            <View className="mb-6">
              ...
              <View className="flex-row items-center rounded-xl border border-slate-200 bg-slate-50 px-3">
                ...
                <TextInput className="ml-3 flex-1 py-3.5 text-base font-medium text-slate-900" placeholder="Tên khách hàng" placeholderTextColor={Colors.slate400} value={name} onChangeText={setName}/>
              </View>
            </View>
            ...
            <View className="mb-6">
              ...
              <View className="flex-row items-center rounded-xl border border-slate-200 bg-slate-50 px-3">
                ...
                <TextInput className="ml-3 flex-1 py-3.5 text-base font-medium text-slate-900" placeholder="name@example.com" placeholderTextColor={Colors.slate400} keyboardType="email-address" autoCapitalize="none" value={email} onChangeText={setEmail}/>
              </View>
            </View>
            ...
            <View className="mb-6">
              ...
              <View className="flex-row items-center rounded-xl border border-slate-200 bg-slate-50 px-3">
                ...
                <TextInput className="ml-3 flex-1 py-3.5 text-base font-medium text-slate-900" placeholder="Địa chỉ khách hàng" placeholderTextColor={Colors.slate400} value={address} onChangeText={setAddress}/>
              </View>
            </View>
            ...
            <View className="mb-6">
              ...
              <View className="items-start rounded-xl border border-slate-200 bg-slate-50 px-3">
                ...
                <TextInput className="ml-8 flex-1 py-3.5 pr-2 text-base font-medium text-slate-900 min-h-[80px]" placeholder="Ghi chú thêm (không bắt buộc)" placeholderTextColor={Colors.slate400} multiline textAlignVertical="top" value={note} onChangeText={setNote}/>
              </View>
            </View>
          </View>
