...
import { ArrowLeft, User, Phone, Envelope, MapPin, PencilSimple, CircleDashed, PlayCircle, StopCircle, } from 'phosphor-react-native';
import { useAppDispatch, useAppSelector } from '@/app/store';
import { fetchCustomerByIdThunk, updateCustomerStatusThunk, clearSelectedCustomer, selectCustomerById, } from '../customersSlice';
import { Colors, shadowCard, shadowCTA, shadowFloating, pressedStyleSmall, labelStyle, layoutContainer } from '@/theme/tokens';
function InfoRow({ icon: Icon, label, value }) {
    if (!value)
        return null;
    return (<View className="mb-4 flex-row items-start gap-3">
...
export default function CustomerDetailScreen({ navigation, route }) {
    ...
    const handleToggleStatus = useCallback(() => {
        ...
        Alert.alert(isSuspended ? 'Mở khóa khách hàng' : 'Khóa khách hàng', isSuspended
    ...
    return (<SafeAreaView className="flex-1 bg-page">
      <View className="flex-row items-center gap-3 px-4 py-3">
        ...
        <Pressable onPress={() => navigation.navigate('CustomerForm', { customerId: customer._id })} className="h-10 w-10 items-center justify-center rounded-xl bg-white" style={({ pressed }) => [shadowFloating, pressedStyleSmall(pressed)]}>
      ...
      <ScrollView contentContainerStyle={layoutContainer} contentContainerClassName="px-6 pb-12 pt-4">
        ...
        <View className="mb-6 items-center">
          ...
          <View className="mt-3 flex-row gap-2">
            {customer.status === 'active' ? (<View className="rounded bg-green-50 px-2.5 py-1">
                <Text className="text-xs font-bold text-green-700 uppercase">HOẠT ĐỘNG</Text>
              </View>) : (<View className="rounded bg-red-50 px-2.5 py-1">
                <Text className="text-xs font-bold text-red-700 uppercase">ĐÃ KHÓA</Text>
              </View>)}
            {customer.isVerified && (<View className="rounded bg-indigo-50 px-2.5 py-1">
