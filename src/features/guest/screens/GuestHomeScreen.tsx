// src/features/guest/screens/GuestHomeScreen.tsx
...
import { AuthStackParamList } from "@/app/navigation"; // Assuming this will be renamed or reused for Guest flow
import {
  Colors,
  shadowCard,
  shadowOutline,
...
// ─── Service card ────────────────────────────────────────────────
function GuestServiceCard({ service }: { service: Service }) {
...
export default function GuestHomeScreen({ navigation }: Props) {
      ...
      >
        ...
        <View className="w-full gap-3 mt-2">
          ...
          <Pressable
            ...
            className="flex-row items-center justify-center gap-2 rounded-xl bg-slate-900 px-6 py-4"
            style={({ pressed }) => [shadowCTA, pressedStyle(pressed)]}
          >
  ...
  return (
    <SafeAreaView className="flex-1 bg-page">
      <FlatList
        data={list}
        renderItem={({ item }) => <GuestServiceCard service={item} />}
