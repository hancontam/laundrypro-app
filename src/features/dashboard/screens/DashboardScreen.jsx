...
import { Colors, shadowFloating, pressedStyleSmall, layoutContainer, } from "@/theme/tokens";
...
export default function DashboardScreen({ navigation }) {
    ...
    const renderContent = () => {
        if (isLoading && !refreshing) {
            return (<View className="flex-1 px-6 pt-6">
          <DashboardSkeleton />
        </View>);
        }
        if (error && !stats) {
        ...
        return (<ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }} className="flex-1 px-6 pt-6" refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh}/>}>
    ...
    return (<SafeAreaView className="flex-1 bg-page" edges={["top", "bottom"]}>
      ...
      <View className="flex-row items-center justify-between px-6 pt-4 pb-2 z-10">
        ...
        {!isLoading && !error && (<Pressable className="h-10 w-10 items-center justify-center rounded-xl bg-white" style={({ pressed }) => [shadowFloating, pressedStyleSmall(pressed)]}>
