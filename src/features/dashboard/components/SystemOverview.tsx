...
export default function SystemOverview({ revenue, byStatus = [] }: Props) {
  ...
  return (
    <View className="flex-row flex-wrap justify-between mb-6">
      {cards.map((card, index) => (
        <View
          key={index}
          className="mb-4 w-[48%] rounded-2xl border border-slate-100 bg-white p-4"
          style={shadowCard}
        >
          <View
            className={`mb-3 h-10 w-10 items-center justify-center rounded-full ${card.bg}`}
          >
            {card.icon}
          </View>
