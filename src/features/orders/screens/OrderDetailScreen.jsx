...
import { ArrowLeft, User, CreditCard, CheckCircle, } from "phosphor-react-native";
...
export default function OrderDetailScreen({ navigation, route }) {
    ...
    const canUpdateStatus = selectedOrder?.status === "pending" &&
    ...
    const handleCompleteOrder = useCallback(() => {
        if (!selectedOrder)
            return;
        Alert.alert("Hoàn thành đơn hàng", "Bạn có chắc muốn đánh dấu đơn hàng này là hoàn thành?", [
    ...
    return (<SafeAreaView className="flex-1 bg-page">
      ...
      <View className="flex-row items-center gap-3 px-4 py-3">
        ...
        {order && (<View className={`rounded-lg px-2.5 py-1 ${ORDER_STATUS[order.status].bg}`}>
      ...
      {order && (<ScrollView contentContainerStyle={layoutContainer} contentContainerClassName="px-6 pb-8">
          ...
          {canUpdateStatus && (<Pressable onPress={handleCompleteOrder} disabled={isLoading} className="flex-row items-center justify-center rounded-xl bg-slate-900 py-4" style={({ pressed }) => [
                    shadowCTA,
