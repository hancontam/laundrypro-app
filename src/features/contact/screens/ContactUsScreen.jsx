...
import { Colors, shadowFloating, shadowCTA, pressedStyle, pressedStyleSmall, layoutContainer, labelStyle, } from "@/theme/tokens";
export default function ContactUsScreen({ navigation }) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            Alert.alert("Thành công", "Tin nhắn của bạn đã được gửi. Chúng tôi sẽ liên hệ trong thời gian sớm nhất!", [{ text: "OK", onPress: () => navigation.goBack() }]);
        }
        catch (err) {
    ...
    return (<SafeAreaView className="flex-1 bg-page">
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} className="flex-1">
        ...
        <View className="flex-row items-center justify-between px-6 pt-4 pb-2">
          <Pressable onPress={() => navigation.goBack()} className="h-10 w-10 items-center justify-center rounded-xl bg-white" style={({ pressed }) => [
            shadowFloating,
          ...
          <View className="w-full">
            ...
            <View className="mb-5">
              ...
              <View className="flex-row items-center rounded-xl border border-slate-200 bg-slate-50 px-3">
                ...
                <TextInput className="ml-3 flex-1 py-3.5 text-base font-semibold text-slate-900" placeholder="Nhập họ và tên" placeholderTextColor={Colors.slate300} value={name} onChangeText={setName} editable={!isLoading}/>
              </View>
            </View>
            ...
            <View className="mb-5">
              ...
              <View className="flex-row items-center rounded-xl border border-slate-200 bg-slate-50 px-3">
                <EnvelopeSimple size={20} color={Colors.slate400} weight="bold"/>
                <TextInput className="ml-3 flex-1 py-3.5 text-base font-semibold text-slate-900" placeholder="Nhập email của bạn" placeholderTextColor={Colors.slate300} keyboardType="email-address" autoCapitalize="none" value={email} onChangeText={setEmail} editable={!isLoading}/>
              </View>
            </View>
            ...
            <View className="mb-5">
              ...
              <View className="flex-row items-center rounded-xl border border-slate-200 bg-slate-50 px-3">
                ...
                <TextInput className="ml-3 flex-1 py-3.5 text-base font-semibold text-slate-900" placeholder="Ví dụ: Cần hỗ trợ đơn hàng" placeholderTextColor={Colors.slate300} value={subject} onChangeText={setSubject} editable={!isLoading}/>
              </View>
            </View>
            ...
            <View className="mb-8">
              ...
              <View className="rounded-xl border border-slate-200 bg-slate-50 p-3 pt-3 flex-row items-start">
                <ChatCircleText size={20} color={Colors.slate400} weight="bold" style={{ marginTop: 2 }}/>
                <TextInput className="ml-3 flex-1 text-base font-semibold text-slate-900" placeholder="Nhập nội dung cần hỗ trợ..." placeholderTextColor={Colors.slate300} multiline numberOfLines={4} style={{ minHeight: 100, textAlignVertical: "top" }} value={message} onChangeText={setMessage} editable={!isLoading}/>
              </View>
            </View>
