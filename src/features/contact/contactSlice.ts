  ...
  async (
    ...
    try {
      ...
      return rejectWithValue(
        err.response?.data?.message || "Không thể tải danh sách liên hệ",
      );
    }
  },
);

export const loadMoreContactsThunk = createAsyncThunk(
  "contact/loadMoreContacts",
  async (_, { getState, rejectWithValue }) => {
    try {
      ...
      if (err === "No more pages") return rejectWithValue(err);
      return rejectWithValue(
        err.response?.data?.message || "Không thể tải thêm liên hệ",
      );
    }
  },
);
    ...
    builder
      ...
      .addCase(loadMoreContactsThunk.fulfilled, (state, action) => {
        ...
        const newContacts = action.payload.contacts.filter(
          (c) => !state.list.find((existing) => existing._id === c._id),
        );
