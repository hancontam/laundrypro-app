...
export const fetchStaffThunk = createAsyncThunk("users/fetchStaff", async (params, { rejectWithValue }) => {
    try {
        ...
        return rejectWithValue(err.response?.data?.message || "Không thể tải danh sách nhân viên");
    }
});
/** Load more staff */
export const loadMoreStaffThunk = createAsyncThunk("users/loadMoreStaff", async (params, { getState, rejectWithValue }) => {
    try {
        ...
        return rejectWithValue(err.response?.data?.message || "Không thể tải thêm nhân viên");
    }
});
/** Fetch user by ID */
export const fetchUserByIdThunk = createAsyncThunk("users/fetchUserById", async (id, { rejectWithValue }) => {
    try {
        const response = await usersService.getUserById(id);
        return response.data;
    }
    catch (err) {
        return rejectWithValue(err.response?.data?.message || "Không thể tải thông tin nhân viên");
    }
});
/** Create staff (Admin) */
export const createStaffThunk = createAsyncThunk("users/createStaff", async (payload, { rejectWithValue }) => {
    try {
        const response = await usersService.createStaff(payload);
        return response.data;
    }
    catch (err) {
        return rejectWithValue(err.response?.data?.message || "Không thể tạo nhân viên");
    }
});
/** Update staff (Admin) - syncs selectedUser + list */
export const updateStaffThunk = createAsyncThunk("users/updateStaff", async ({ id, payload }, { rejectWithValue }) => {
    try {
        const response = await usersService.updateStaff(id, payload);
        return response.data;
    }
    catch (err) {
        return rejectWithValue(err.response?.data?.message || "Không thể cập nhật nhân viên");
    }
});
/** Update user status (Admin) — syncs selectedUser + list */
export const updateUserStatusThunk = createAsyncThunk("users/updateUserStatus", async ({ id, status }, { rejectWithValue }) => {
    try {
