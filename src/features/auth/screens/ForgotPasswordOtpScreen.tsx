...
export default function ForgotPasswordOtpScreen({
  ...
  const cardTitle = useMemo(
    () => (verifiedIdToken ? 'Đặt lại mật khẩu' : 'Xác thực OTP'),
    [verifiedIdToken],
  );
  ...
  const handleVerifyOtp = useCallback(async () => {
    const code = otp.join('');
    if (code.length !== OTP_LENGTH || verifiedIdToken) return;
  ...
  const handleResetPassword = useCallback(async () => {
    if (!verifiedIdToken || !isValidPassword) return;
      ...
      >
        ...
        >
          ...
          {!verifiedIdToken ? (
            <>
              ...
              >
                {isVerifyingOtp ? (
                  ...
                  <Text className="text-base font-bold text-white">Xác nhận OTP</Text>
                )}
              </Pressable>
            </>
          ) : (
            <>
              <View
                className="mb-5 flex-row items-center rounded-2xl border border-green-100 bg-green-50 px-4 py-3"
                style={shadowCard}
              >
