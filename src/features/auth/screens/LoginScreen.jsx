...
import { Colors, shadowCTA, shadowCard, gradient135, pressedStyle, layoutContainer, labelStyle, } from '@/theme/tokens';
        ...
        <ScrollView contentContainerStyle={layoutContainer} contentContainerClassName="flex-grow justify-center px-6 py-8" keyboardShouldPersistTaps="handled">
          ...
          <View className="mb-10 items-center">
            <View className="mb-6">
              ...
              <LinearGradient colors={[Colors.indigo50, Colors.indigo200]} start={gradient135.start} end={gradient135.end} style={{
            width: 80,
            height: 80,
            borderRadius: 40,
            alignItems: 'center',
          ...
          {step === 'phone' && (<View>
              ...
              <View className="mb-6 flex-row items-center rounded-xl border border-slate-200 bg-slate-50 px-3">
                ...
                <TextInput className="ml-3 flex-1 py-3.5 text-base font-semibold text-slate-900" placeholder="0788 876 568" placeholderTextColor={Colors.slate300} keyboardType="phone-pad" value={phone} onChangeText={(t) => setPhone(t.replace(/[^0-9]/g, ''))} autoFocus/>
              </View>

              {/* §5.2 — Full-Width Submit (Dark) */}
              <Pressable onPress={handleCheckLogin} disabled={isAnyLoading || !phone.trim()} className="flex-row items-center justify-center rounded-xl bg-slate-900 py-4" style={({ pressed }) => [
                shadowCTA,
          ...
          {step === 'password' && (<View>
              ...
              {/* §5.2 — Full-Width Submit (Dark) */}
              <Pressable onPress={handlePasswordLogin} disabled={isAnyLoading || !password.trim()} className="flex-row items-center justify-center rounded-xl bg-slate-900 py-4" style={({ pressed }) => [
                shadowCTA,
                pressedStyle(pressed),
