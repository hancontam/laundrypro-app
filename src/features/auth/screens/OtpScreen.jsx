...
export default function OtpScreen({ navigation, route }) {
    ...
    const handleVerifyOtp = useCallback(async (code) => {
        ...
        try {
            ...
            if (!user)
                throw new Error('Không thể lấy thông tin user xác thực.');
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1">
        ...
        <ScrollView contentContainerStyle={layoutContainer} contentContainerClassName="flex-grow justify-center px-6 pb-8" keyboardShouldPersistTaps="handled">
          ...
          <Pressable onPress={() => handleVerifyOtp(otp.join(''))} disabled={isLoading || otp.some((d) => !d)} className="flex-row items-center justify-center rounded-xl bg-slate-900 py-4" style={({ pressed }) => [
            ...
            {isLoading ? (<ActivityIndicator color="#fff"/>) : (<Text className="text-base font-bold text-white">Xác nhận</Text>)}
          </Pressable>
        </ScrollView>
