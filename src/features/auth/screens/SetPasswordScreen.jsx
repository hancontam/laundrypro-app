...
export default function SetPasswordScreen() {
    ...
    const handleSetPassword = useCallback(async () => {
        if (!isValid)
            return;
        dispatch(clearError());
        const result = await dispatch(setPasswordThunk({ password, confirmPassword }));
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1">
        <ScrollView contentContainerStyle={layoutContainer} contentContainerClassName="flex-grow justify-center px-6 pb-8" keyboardShouldPersistTaps="handled">
          ...
          <View className="mb-6">
            ...
            <View className="flex-row items-center rounded-xl border border-slate-200 bg-slate-50 px-3">
              <LockSimple size={20} color={Colors.slate400} weight="bold"/>
              <TextInput className="ml-3 flex-1 py-3.5 text-base font-semibold text-slate-900" placeholder="Nhập lại mật khẩu" placeholderTextColor={Colors.slate300} secureTextEntry={!showConfirm} value={confirmPassword} onChangeText={setConfirmPassword}/>
          ...
          <Pressable onPress={handleSetPassword} disabled={isLoading || !isValid} className="flex-row items-center justify-center rounded-xl bg-indigo-600 py-4" style={({ pressed }) => [
            shadowCTA,
            pressedStyle(pressed),
            { opacity: isLoading || !isValid ? 0.5 : 1 },
        ]}>
            {isLoading ? (<ActivityIndicator color="#fff"/>) : (<Text className="text-base font-bold text-white">
                Xác nhận mật khẩu
              </Text>)}
          </Pressable>
        </ScrollView>
