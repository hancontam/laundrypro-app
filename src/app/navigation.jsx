...
function TabItem({ icon: Icon, focused }) {
  return (
    <View className="items-center justify-center">
      <View
        className={`h-10 w-10 items-center justify-center rounded-2xl ${focused ? "bg-indigo-50" : "bg-transparent"}`}
      >
        <Icon
          size={20}
...
function ShortcutCard({
  ...
  return (
    <Pressable
      onPress={onPress}
      className={`${compact ? "rounded-[22px] p-3.5" : "rounded-2xl p-4"} border border-slate-100 bg-white`}
      style={({ pressed }) => [
        shadowCard,
        pressedStyle(pressed),
        { width: cardWidth, minHeight: compact ? 148 : 164 },
      ]}
    >
      <View
        className={`${compact ? "mb-3 h-10 w-10 rounded-xl" : "mb-4 h-11 w-11 rounded-2xl"} items-center justify-center bg-indigo-50`}
      >
        <Icon size={compact ? 20 : 22} color={Colors.indigo600} weight="bold" />
      </View>
      <Text
        className={`${compact ? "text-[15px]" : "text-sm"} font-extrabold text-slate-900`}
        numberOfLines={2}
      >
        {title}
      </Text>
      <Text
        className={`mt-1 ${compact ? "text-[11px] leading-4" : "text-xs leading-5"} font-medium text-slate-500`}
        numberOfLines={2}
      >
        {subtitle}
      </Text>
...
function HomeScreen({ navigation }) {
    ...
    ...(isStaffOrAdmin
      ? [
          ...
          },
          {
            ...
            title: "Quản trị",
            subtitle:
              "Khách hàng, thanh toán và các công cụ quản lý phù hợp với quyền của bạn.",
  ...
  const availableShortcutWidth = Math.max(shortcutContainerWidth, 0);
  const shortcutCardWidth =
    availableShortcutWidth > 0 ? availableShortcutWidth : 0;
  const compactShortcuts = false;
  return (
    <SafeAreaView className="flex-1 bg-page">
      ...
      >
        ...
        <View
          ...
          onLayout={(event) => {
            setShortcutContainerWidth(event.nativeEvent.layout.width);
          }}
        >
  ...
  return (
    ...
    >
      <Tab.Screen
        ...
        options={{
          tabBarIcon: ({ focused }) => (
            <TabItem icon={House} focused={focused} />
          ),
        }}
      />
      <Tab.Screen
        ...
        options={({ route }) => ({
          tabBarIcon: ({ focused }) => (
            <TabItem icon={ClipboardText} focused={focused} />
          ),
      ...
      <Tab.Screen
        ...
        options={({ route }) => ({
          tabBarIcon: ({ focused }) => (
            <TabItem icon={Broom} focused={focused} />
          ),
          tabBarStyle: resolveTabBarStyle(route, [
            "ServiceDetail",
            "ServiceForm",
      ...
      {isStaffOrAdmin && (
        <Tab.Screen
          ...
          options={({ route }) => ({
            tabBarIcon: ({ focused }) => (
              <TabItem icon={UsersThree} focused={focused} />
            ),
            tabBarStyle: resolveTabBarStyle(route, [
              "Dashboard",
              "CustomerList",
